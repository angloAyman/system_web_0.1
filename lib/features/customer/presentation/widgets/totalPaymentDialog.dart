...
class _PaymentDialogState extends State<PaymentDialog> {
    ...
    return AlertDialog(
      ...
      content: Column(
        ...
        children: [
          ...
          Expanded(child: _buildBillTable(widget.customerBills, _updatedPayments)),
          SizedBox(height: 10),
  ...
  Widget _buildBillTable(List<Bill> bills, Map<int, double> updatedPayments) {
    return SingleChildScrollView(
      scrollDirection: Axis.horizontal, // للسماح بالتمرير الأفقي إذا لزم الأمر
      child: SingleChildScrollView(
        scrollDirection: Axis.vertical, // للسماح بالتمرير العمودي
        child: DataTable(
          columnSpacing: 12.0,
          columns: [
          ...
          rows: bills.map((bill) {
            double remainingAmount = bill.total_price - (updatedPayments[bill.id] ?? 0);
            return DataRow(cells: [
              DataCell(Text(bill.id.toString())),
              DataCell(Text('${bill.total_price.toStringAsFixed(2)} L.E')),
              DataCell(Text('${(updatedPayments[bill.id] ?? 0).toStringAsFixed(2)} L.E')),
              DataCell(
                Text(
                  '${remainingAmount.toStringAsFixed(2)} L.E',
