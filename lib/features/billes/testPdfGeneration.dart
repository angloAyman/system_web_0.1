...
//   pdf.addPage(pw.Page(build: (pw.Context context) => pw.Center(child: pw.Text('Test PDF لالتلتلن'))));
//
...
//   try {
//     // تحميل الخط العربي
//     final font = await rootBundle.load("assets/font/Amiri-Regular.ttf");
...
//
//     // إضافة شعار (اختياري)
//     final logoImage = await rootBundle.load('assets/logo/logo-1.jpeg'); // استبدل بـ مسار الشعار الخاص بك
...
//           base: ttf,
//         ),
//         textDirection: pw.TextDirection.rtl, // تفعيل الكتابة من اليمين لليسار
//
...
//                  pw.Column(
//                      children: [
//                        pw.Text('ROTOSH',style: pw.TextStyle(fontSize: 24, ),),
//                        pw.Text('فاتورة',style: pw.TextStyle(fontSize: 24, ),)
//                      ],),
//                 ],
//               ),
...
//               pw.Text('Customer Name: ${bill.customerName}', style: pw.TextStyle(fontSize: 16)),
//               pw.Text('Date: ${bill.date}', style: pw.TextStyle(fontSize: 16)),
//               pw.SizedBox(height: 16),
//
//               // جدول العناصر
//               pw.Text('Items:', style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold)),
//               pw.Table.fromTextArray(
//                 headers: ['Category', 'Subcategory', 'Amount', 'Price', 'Total'],
//                 data: bill.items.map((item) {
//                   return [
//                     item.categoryName,
//                     item.subcategoryName,
//                     item.amount.toString(),
//                     '\"جنيه"${item.price.toStringAsFixed(2)}',
//                     '\"جنيه"${(item.amount * item.price).toStringAsFixed(2)}',
//                   ];
//                 }).toList(),
//               ),
//               pw.SizedBox(height: 16),
...
//               // المجموع النهائي
//               // pw.Text(
//               //   'Total: \$${bill.items.fold(0, (sum, item) => sum + (item.amount * item.price)).toStringAsFixed(2)}',
...
//     // تحديد مسار التخزين
//     final directory = await getApplicationDocumentsDirectory();
//     final file = File('${directory.path}/arabic_text(${bill.customerName}).pdf');
//
//     // حفظ ملف PDF
//     await file.writeAsBytes(await pdf.save());
//
//     print('PDF تم حفظه في: ${file.path}');
//
//     ScaffoldMessenger.of(context).showSnackBar(
//       SnackBar(content: Text('PDF تم حفظه في: ${file.path}')),
//     );
//   } catch (e) {
